...
# By HarJIT in 2019/2020/2022/2025.
...
def read_single_byte(fil, *, mapper=identitymap, typ="plainext", filter_to_single=False, cidmap=None, cidmap_extra=None):
    column_indices = None
    for _i in open(os.path.join(directory, fil), "r", encoding="utf-8"):
        ...
        if not _i.strip():
            ...
            if not column_indices:
                columns = _i.rstrip().split("\t")
                source = columns.index(cidmap[0])
                dest = columns.index(cidmap[1])
                column_indices = source, dest
                continue
            else:
                columns = _i.rstrip().split("\t")
                byts = [i for i in columns[column_indices[0]].split(",") if not i.endswith(("*", "v"))]
                ucs = [i for i in columns[column_indices[1]].split(",") if not i.endswith(("*", "v"))]
                if cidmap_extra and not ucs and columns[0] in cidmap_extra:
                    ucs = cidmap_extra[columns[0]]
                elif not byts or not ucs:
                    continue
                else:
                    ucs = ucs[0]
        # TODO fix repeated code here.
            ...
            byts, ucs = _i.split("\t", 2)[:2]
            pointers = [int(byts.strip(), 10)]
        #
        if pointers is None:
            pointers = []
            for b in byts:
                if typ == "plainext":
                    pointer = int(b, 16) - 0x80
                    if pointer < 0:
                        continue
                elif typ == "plainext7":
                    pointer = int(b, 16)
                    ...
                    pointer = int(b, 16) - 0x21
                    if (pointer < 0) or (pointer > 93):
                        continue
                elif typ == "GR94":
                    pointer = int(b, 16) - 0xA1
                    if (pointer < 0) or (pointer > 93):
                        continue
                elif typ == "GR96":
                    pointer = int(b, 16) - 0xA0
                    if pointer < 0:
                        continue
                elif typ == "CL33":
                    pointer = int(b, 16)
        ...
        for pointer in pointers:
            if orig_ucs[:2] in ("0x", "U+", "<U"):
                ucs = mapper(pointer, tuple(int(j, 16) for j in orig_ucs[2:].rstrip(">").replace("><U", "+").split("+")))
            else:
                ucs = mapper(pointer, tuple(int(j, 16) for j in orig_ucs.split("+")))
            #
            if len(_temp) > pointer:
                if _temp[pointer]:
                    # Favour the earlier listed mapping unless it has a compatibility decomposition
                    already_there = chr(_temp[pointer][0])
                    if (already_there in namedata.compat_decomp) or (
                            already_there in namedata.canonical_decomp):
                        continue
                _temp[pointer] = ucs
            else:
...
def derive_supplementary_cid_mapping(fil, column, data, typ):
    table = {}
    column_index = None
    for _i in open(os.path.join(directory, fil), "r", encoding="utf-8"):
        pointer = None
        if not _i.strip():
            ...
            if columns[column_index] != "*":
                for _j in columns[column_index].split(","):
                    offset = {
                        "plainext7": 0, "GL94": 0x21, "plainext": 0x80, "GL96": 0xA0, "GR94": 0xA1,
                    }[typ]
                    idx = int(_j, 16) - offset
                    if idx >= 0 and idx < len(data):
                        if data[idx]:
                            table.setdefault(columns[0], "+".join(f"{_k:08x}" for _k in data[idx]))
    return table
